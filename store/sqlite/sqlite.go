...
func (s Sqlite) Save(ctx context.Context, b store.Bookmark) error {
	stmt, err := s.db.PrepareContext(ctx, `INSERT INTO bookmarks (text, user) VALUES (?, ?)`)
	if err != nil {
	...
	_, err = stmt.Exec(b.Text, b.User)
	return err
}

func (s Sqlite) Delete(ctx context.Context, b store.Bookmark) error {
	stmt, err := s.db.PrepareContext(ctx, `DELETE FROM bookmarks WHERE text = ? AND user = ?`)
	if err != nil {
...
func (s Sqlite) PickRandom(ctx context.Context, user int) (store.Bookmark, error) {
	...
	stmt, err := s.db.PrepareContext(ctx, `SELECT text, user FROM bookmarks WHERE user = ? ORDER BY RANDOM() LIMIT 1`)
	if err != nil {
		return b, err
	}
	defer func(stmt *sql.Stmt) {
	...
	if err = stmt.QueryRow(user).Scan(&b.Text); err != nil {
		return b, err
	}
